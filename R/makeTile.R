...
#message(length(which(tempTilecat[,4]==9)))
...
message(paste('Filename for next configuring: ',filename,'.fld',sep=''))
...
message(paste('Total Objects sent to configure: ',length(subset[,1]),sep=''))
...
if(magpri==FALSE){}else{subset[subset[,5]>magpri & subset[,5]<magpri+maglim,4]=subset[subset[,5]>magpri & subset[,5]<magpri+maglim,4]+1;message(summary(subset[,5]));message(summary(subset[,4]))}
...
runCONFIG(long=subset[,1],lat=subset[,2],id=subset[,3],prior=subset[,4],mags=subset[,5],IPorig=IPorig,file=filename,label=filename,tilecens=tilecens[i,],basedir=basedir,directory=directory, configdir=configdir, manual=manual, plate=plate, date=date, append_letter=append_letter)
...
if(length(which(FibVec==0))>5 & length(subset[,1]>550) & manual==FALSE & medshift==TRUE){
	...
	message(paste('BAD FIELD. Shifting tile centre to',tilecens[i,1],'RA/deg',tilecens[i,2],'Dec/deg and reconfiguring.'))
